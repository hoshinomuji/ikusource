...
export default async function AdminServicesPage() {
    ...
    const configsDataRaw = await db.select({
        id: directAdminConfig.id,
        panelUrl: directAdminConfig.panelUrl,
        serverIp: directAdminConfig.serverIp,
        resellerUsername: directAdminConfig.resellerUsername
    }).from(directAdminConfig)

    const configsData = configsDataRaw.map(c => ({
        id: c.id,
        label: `${c.resellerUsername} @ ${c.serverIp}`,
        panelUrl: c.panelUrl
    }))

    return <AdminHostingClient
        orders={orders}
        usersForImport={usersData}
        packagesForImport={packagesData}
        configsForImport={configsData}
    />
}
